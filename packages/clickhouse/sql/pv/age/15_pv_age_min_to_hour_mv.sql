CREATE MATERIALIZED VIEW IF NOT EXISTS lightscope.pv_age_min_to_hour_mv
TO lightscope.pv_age_hour AS
SELECT
    tenant_id_hash,
    ...
    now() as created_at,
    now() as updated_at
FROM lightscope.pv_age_min
GROUP BY tenant_id_hash, date, site_name, url_hash, age;