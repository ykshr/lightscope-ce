...
* Guidance on where to place different types of code
  - In packages/web, the calendar.tsx shadcn/ui component and related inputs (like DateBeforeAfterInput.tsx) are sensitive to react-day-picker version upgrades. Upstream changes (e.g., from v9 to v10) introduce breaking API changes like the removal of getDefaultClassNames and renaming table to month_grid, which require application-level migration when updating.
