...
#### Restrictions
* Guardrails such as:
  - When writing temporary Playwright verification scripts to `/home/jules/verification/`, ensure the directory exists first using `mkdir -p /home/jules/verification` to prevent file writing errors.
  - When implementing performance optimizations (even if under the 'Bolt' persona), do not use the `⚡ Bolt:` prefix in code comments. Keep the code comments clean. The prefix is only required for the Pull Request title.
  - When working on the codebase, never modify `package.json` or `tsconfig.json` without explicit instruction, do not make breaking changes, and do not sacrifice code readability for micro-optimizations.
  - When a task targets documentation only, do not modify any actual code, even if CI errors occur, unless explicitly requested.
