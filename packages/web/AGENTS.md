...
- **Library Restrictions**: Do not introduce libraries like `axios`, `redux`, or `zustand`. Use `fetch` and TanStack Query. Icons are implemented using the `@phosphor-icons/react` library. Ensure any new icon additions import from this package.
- **Data Fetching Rules**:
...
- **Start Development Server**:
  ...
  ```
  *(Note: The Vite development server is configured to run on port 3000 by default. Point frontend verification scripts to `http://localhost:3000`)*
...
#### Project Structure
- `src/components/`: Reusable UI components (including `shadcn/ui`). Charting components in `src/components/charts/` are structured with main components utilizing reusable templates in `templates/` and data processing logic in `helpers/`.
- `src/pages/`: Page components corresponding to routes.
- `src/helpers/`: Utility functions and authentication (`better-auth` client). When configuring frontend `better-auth` plugins (like `organizationClient()`) in `packages/web/src/helpers/auth.ts`, ensure the corresponding backend plugin (e.g., `organization()`) is actively configured in the API (`packages/api/src/helpers/auth.ts`) to avoid 404 API errors.
