...
- **Library Restrictions**: Do not introduce libraries like `axios`, `redux`, or `zustand`. Use `fetch` and TanStack Query. Icons in the `packages/web` frontend application are implemented using the `@phosphor-icons/react` library. Ensure any new icon additions import from this package.
- **Data Fetching Rules**:
...
- **Guidance on code placement**: Place standard React component logic inside `src/components/` and define unique views per route in `src/pages/`. Use `src/hooks/` for generic abstractions.
